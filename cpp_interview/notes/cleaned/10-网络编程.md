...
![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1650594287168-c932dcdc-eb28-481d-a062-a4aeca1c961b.png)
大小端转换
```c
#include <stdio.h>
/*联合*/
...
#### 基于TCP的网络通信
![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1650607487072-80d0af01-06db-4142-ade1-b910577f5683.png)
网络的用法和文件一样。

### `socket`
$ man 2 socket
![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1650637305018-921ccf87-b5c1-4df2-bec5-d2ddb8502a18.png)
...
Linux 下抓包 tcpdump
先 socket 指定服务端地址， 调用 connect  第一次握手

TCP连接的标志
如何区分两条不同的TCP连接
**四元组      < 源IP，源端口，目的IP，目的端口 >**
服务端的IP和端口一般是固定的
一个TCP连接最多可以创建65535 个端口？(  **X  **)

`bind` - bind a name to a socket
#include <sys/types.h>          /* See NOTES */
#include <sys/socket.h>
int bind(int sockfd, const struct sockaddr *addr, socklen_t addrlen);
绑定一个IP给socket 一般是服务端使用
```c
#include <43func.h>
int main(int argc,char *argv[]){
    // ./connect 192.168.14.9 1234
...
![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1650638594629-7ff18b93-465c-4416-b2b2-aed9814ccc54.png)
`listen`是说明让socket当作服务端来使用
`listen`是清空socket的文件对象，建立了半连接队列和全连接队列
65535  65536  mss
```c
#include <43func.h>
int main(int argc,char *argv[]){
...
后两个全填NULL，不获取客户端信息
_value_result  传入传出参数_
![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1650638782183-00c83d7c-6b81-4f17-bc80-d3b6a47c1613.png)
accept从**listen socket**的全连接队列中取出一个连接（**sockFd  建立连接**），构建一个新的已连接socket （**netFd 发送数据**）
accept的本质是一个read，若连接队列为空，accept就会阻塞
```c
#include <43func.h>
int main(int argc,char *argv[]){
    // ./server 192.168.14.9 1234
...
**只能操作socket**
send和write是**拷贝数据到SND缓冲区**，后续操作由**内核协议栈**进行。
...
**只能操作socket**
recv和read是**拷贝数据从RECV缓冲区到用户空间**，后续操作由**内核协议栈**进行。
...
```
send放入缓冲区，实际发送由内核协议栈发送
TCP是一种字节流协议，消息与消息之间无边界
MSS对用户层无影响，由内核控制发送 MSS ( 1460 )

如何使接收方和发送方解读相同的信息 - 用户层自定义协议

### 利用socket实现即时聊天
基本上和管道一样，socket的R，W的fd是同一个
...
主动关闭方在四次挥手后会进入TIME_WAIT状态
客户端会换端口，服务端因为bind会等待2MSL
**服务端**如果**主动关闭**，会进入**TIME_WAIT，固定2MSL（60s)**，服务端在TIME_WAIT状态，不允许bind同样的IP和端口。

#### 让服务端无视TIME_WAIT状态
修改socket的属性（ 在socket创建之后，bind之前）
...
#### 如何支持断线重连
用select的rdset 监听服务端的sockFd 读事件
如果sockFd就绪，说明服务端调用accept不会阻塞
```c
#include <43func.h>
int main(int argc, char *argv[]){
    // ./client 192.168.14.9 1234
...
TCP和UDP收到消息个数的区别：
 主机A向主机B发送数据，以TCP方式发送3个包，对方可能会收到几个包？
答：任意个
以UDP方式发送3个包，对方可能会收到几个包？
答：至多3个
...
监听集合无大小限制
红黑树 数据结构
![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1650685762233-dfbb13bb-9129-41b1-9d5b-e0bdca7002ce.png)

2. 监听和就绪分离

就绪事件用**一个单独的就绪事件队列**保存
只需 将**就绪事件队列**从内核态**拷贝**到用户态
优点：
...
![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1650856326558-85485684-0e6e-4bfe-ae31-f69267241250.png)

- 同步阻塞

![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1650856478630-3a8c3835-a7df-4c9d-b8ea-30c3fad89550.png)

- 同步非阻塞

![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1650856492550-0f2d1fbc-4bbe-43b0-94e0-8bbcb8cc963d.png)

-  IO多路复用  （ + 非阻塞  目前主流服务器）
- ![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1650856601682-9047bb8d-808f-4c05-8d85-5d8cdbee149a.png)
...
```
工作常用方法：
MSG_DONTWAIT  // 临时设置一次**recv 非阻塞**

epoll的触发方式

- 水平触发

![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1650858303844-a3d067a1-2f6d-4537-b52a-beec2ee041da.png)
某一个客户有大量数据，epoll会重复就绪

- 边缘触发

![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1650858316938-0c1e6de3-15d8-4ae7-b77b-de95a16d086a.png)

如何实现既要epoll就绪次数少，又要把数据读完？
用**epoll的边缘触发** + **while(){ 非阻塞recv; }    （ 面试常面 ）**
```c
#include <43func.h>
int main(int argc, char *argv[]){
    // ./server 192.168.14.9 1234
...
| select | epoll |
| --- | --- |
| fd_set rdset; / FD_ZERO(&rdset); / 放在循环内 | int epfd = epoll_create(1); / 放在循环外 |
| FD_SET(fd, &rdset); / 放在循环内 / 不能直接用于就绪后的事件，需单独设置monitorSet | struct epoll_event event; / event.data.fd = fd; / event.events = EPOLLIN; / epoll_ctl(...,EPOLL_CTL_ADD,...) / 可以直接加新的监听fd |
| 用FD_ISSET( )轮询 | for(  ){ / 遍历就绪事件队列 / readyArr[i].data.fd  / } |
