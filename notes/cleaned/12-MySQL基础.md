...
![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1651146097484-6af5465c-ba67-49b5-bbaf-799bc53fdf9c.png)
创建数据库 ：mysql> create database Dbname;
删除数据库 ：mysql> drop database Dbname;

#### 字符集：
修改数据库字符集：
mysql> alter database DbName default chharacter set gbk;
字符校对集：
mysql> show character set; // 查看数据库支持的字符集
mysql> show collation; // 查看相应字符集的校对规则
...
mysql常用数据类型
![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1651305299419-94754dc8-f4f6-4645-bd86-f77af030d6b9.png)
** TIMESTAMP 在进行insert/update 自动记录  **
...
select 语句中出现的** 列 **不一定真的存在数据表中
相关接口：
mysql > select database();
mysql > select  name, chinese + 10 as chinese, curdata(） from student ；
mysql > select  curdata( );
...
先使用第一个field排序，相同再使用后一个field排序，以此类推
select * from student order by math, english, chinese[asc/desc]

按总成绩进行降序
mysql -> select id, name, chinese + english + math as total from student  order by total DESC;

###  ！分页查询 limit   常考  ( Oracal   sock? )
limit num      一页几行数据

偏移量 与 查看行数
limit  num1 num2    从num1开始（不含第num1行）继续查看num2行
limit  num1 offset num2     从num2开始不含第num2行）查看num1行
...
删除主键约束：
一般情况下，一张表只有一个主键
需要先解除 自动增长 约束，才能 删除 primary key
...
关系数据库通过**外键**可以实现一对多、多对多和一对一的关系；
外键既可以通过数据库来约束，也可以不设置约束，仅依靠应用程序的逻辑来保证。
通过对数据库表创建**索引**，可以提高查询速度。
通过创建唯一索引，可以保证某一列的值具有唯一性；
数据库索引对于用户和应用程序来说都是透明的。
[参考：MySQL 主键、外键、索引](https://zhuanlan.zhihu.com/p/64368422)
...
#### DQL复杂查询
![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1651305453968-322b49ba-f04f-4f09-87bd-a06929955425.png)
...
左表中的数据全部出现， 当右表中没有与之对应的记录时，全部用 NULL代替。
3.2 右外连接： right outer join    **on**
右表中的数据全部出现， 当左表中没有与之对应的记录时，全部用 NULL代替。
...
联合查询：union  并集
合并两条查询语句的查询结果，**去掉其中的重复数据行**， 返回没有重复数据行的查询结果。
```sql
