...
#### workflow是异步框架
抽象机制 ：  “ 任务 ” 一段将要异步执行的代码
![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1657199050351-2c062719-5fa9-45c2-859f-a5a3e78efbe4.png)
机制 ： 异步 + 回调

#### 任务的 “ 串并联 ”
![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1657199070308-2cfcd4f8-3ffa-459e-8009-092fd0c59dbb.png)
workflow 提供了将任务串联（先后）和并联（并发）
...
### redis任务
![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1657200490324-641293d8-5279-4c9b-a84b-4c28e79043ba.png)
...
![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1657200510951-14c10339-f687-4bbd-8400-c93931c877f7.png)
...
![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1657200530665-6c560171-73f2-4509-accf-ba2c52dfe7bb.png)
...
1、 只有客户端才可以调用start方法。
2、 start方法的底层逻辑就是根据本任务对象创建一个序列
3、其中本任务是序列当中的第一个任务，随后启动该任务。
...
#### 序列总结
![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1657202800802-83c058b8-dcac-451b-a1dd-e6f0b6491c96.png)

#### 在序列中使用context（ user_data属性 )共享数据
![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1657202831394-d43f16e4-0f80-4a24-bbc4-74192282355d.png)

 1、需要在任务之间设置明确的先后依赖关系，此时可以采用**系列**将这些任务串联起来，系列可以认为是任务构成的队列，队列内部的任务按照先进先出的方式执行。
2、系列内部可以设置 **context**属性，用于在系列内部的任务之间传递数据；
3、系列还可以传入一个**callback**，这个callback会在系列中所有的任务完成之后调用。
![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1657281212397-0ab201d3-364b-4aa4-8cca-03942488cd06.png)

不同的系列之间是可以并行执行，**每个任务**单独启动的时候会**创建一 个系列**，所以任务之间的并行执行其本质就是系列之间的并行执行。

复习
![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1657286110627-39cb42b4-1420-453b-befd-12e5b9d18b40.png)

### 3. 并行任务   ParallelWork
 与SeriesWork对应的ParallelWork类，描述了一个并行任务，并行任务由序列构成，代表**若干个序列**的**并行执行**。所有序列结束，则这个并行任务的基本工作结束，随后执行这个并行任务所在序列相应的回调函数。
需要特别注意的是，ParallelWork本身也是一种任务，所以它可以加入到其他序列中——（而这个 序列又可以用来构建更加复杂的并行任务）
...
下面是涉及到的接口：
首先是创建任务所需要的工厂函数：
当创建一个任务之后，可以使用Workflow::create_series_work函数在不执行的情况下创建序列。
```cpp
...
并行任务类ParallelWork中，存在add_series方法，可以将某个序列加入到并行任务中。
```cpp
...
#### 一个较复杂的并行的例子
![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1657286597288-cc1e03ee-de80-4335-84b2-a3cb8dece110.png)
...
### 1、简单的http服务器
![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1657286719949-17d56d83-a6f9-47a4-91e0-e8dc27c2931d.png)
服务端任务 ：客户端接入后，服务端框架生成的任务

#### process函数对象 ( 用户设计的 )
作用：去找到serverTask
![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1657286746656-9caf8c41-8c51-475f-9b06-7e87ba58cad3.png)
每当客户端接入，框架解析完请求时，process就调用
![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1657287529461-59f749d4-2adf-473c-b90c-410bb285620f.png)
...
![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1657292118477-7e808dfb-59f2-477f-a04f-9b37e01f74d0.png)
...
serverTask 对客户端总是通过 serverTask->resp 来设置
![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1657293734826-537934fe-0409-44f7-993e-7a495bfd5d04.png)

#### 利用workflow实现反向代理
![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1657293798954-c3861c61-ceeb-4d0f-9f5d-2add1b981a1e.png)
...
#### serverTask
![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1657293787476-681699e5-c54a-4596-8ae6-bdb3eeac6e14.png)

磁盘 稳定（单线程和多线程 速度相同，宜采用 单线程 ）
所有线程都会被**操作系统**维持进**一个队列**。
**顺序写** 速度比较快   磁臂

网络 不稳定，受带宽和网络状况限制（多线程 保证较高速度，不易断联） 多线程 提升带宽的利用率，保证连接稳定性。
顺序写 、 随机写 几乎无差别。

![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1657354494655-5cd50283-db9f-4783-9878-b3a387d436d6.png)
...
![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1657354522728-c0ed4c5e-108f-4f10-ba3e-36294089ef7b.png)
...
`I/O`多路复用  （ Reactor ）：  有数据了，再去处理
异步  （ proactor ）：   处理完数据了 再使用 **回调函数 **通知
理论上，异步 更快 ，测试，二者基本相同。( Windows 没有多路复用 )
...
### 小项目： 超大文件多片上传
分块上传思路 ：
![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1657519516333-68e87fab-d364-46ca-a686-73fb7fcf16b8.png)
...
![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1657519567410-122a46c8-f716-43ba-a80f-4db16d02fb3b.png)
...
2、uppart  ( 不符合RESTful接口标准 ）
/file/mupload/uppart?uploadID=xxx&chkidx=02
报文体 存放文件内容
...
3、complete
![image.png](https://cdn.nlark.com/yuque/0/2022/png/916648/1657519725456-d8dd3d46-e4e7-431a-b378-cfd5f72ec806.png)

1. 解析 uploadID
1. 判断上传是否完成

HGETALL uploadID   统计前缀为chkidx_ 的键的数目
找到chunkCount 对应的值
二者相比较，相等则上传完成，否则，上传尚未完成
...
pimpel  第一次编译好以后，不会再次全量编译
std::chrono C++11时间库

## MySQL 任务

读任务  show / select   `MYSQL_STATUS_OK`
写任务   `MYSQL_STATE_GET_RESULT`
